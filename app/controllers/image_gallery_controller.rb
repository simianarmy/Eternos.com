...
class ImageGalleryController < ApplicationController
  ...
  def show
    ...
    # work-around for goddamn json bug (yes here it is again I fucking hate it)
    ...
    @albums = current_user.contents.photos.collect {|p| p.collection}.compact.uniq.map{|al| al.to_json(:gallery)}.join(',')
    @albums.gsub!('"{', '{')
    @albums.gsub!('}"', '}')
