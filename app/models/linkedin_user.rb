class LinkedinUser < ActiveRecord::Base
  ...
  def process_hash (hash, backup_source_id = nil)
    if !hash['id'].nil?
  ...
  def initialize(peopleprofile,comment_like, cmpies, ncons, backup_source_id)
    ...
    temp = process_hash(@hash, backup_source_id)
    super(@hash)
  ...
  def update_attributes(peopleprofile, comment_like, cmpies, ncons)
    if !comment_like.nil?
