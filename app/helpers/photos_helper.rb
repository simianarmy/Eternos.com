...
module PhotosHelper
  ...
  def thumbnail_link(photo)
    link_to thumb_image_tag(photo), photo.public_filename,
  ...
  def photo_image_tag(photo)
    image_tag(photo.public_filename, :class => "photo", :alt => photo.title, 
        :size => photo.image_size)
  end
  
  def thumb_image_tag(photo)
    thumb = photo.thumbnails.first
    image_tag(photo.public_filename(:thumb), :class => "photo", :alt => photo.title, 
