...
var FancyUpload2 = new Class({
	...
	options: {
		...
		debug: false,
		// MAM 1/27/09 - added these options
		next_step: false,
		object_ids: false,
	...
	fileComplete: function(file, response) {
		...
		if (json.get('status') == 'success') {
			file.element.getElement('a.file-remove').fade('out');
			file.element.addClass('file-success');
			// Append object ID to div value
			file.content_id = json.get('id');
			if (this.options.object_ids)
