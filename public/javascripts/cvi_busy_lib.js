/**
 * cvi_busy_lib.js 1.2 (23-Dec-2008)
...
function getBusy(obj,cl,sz,tp,ir,w,ct,sp,mo) {
	...
	function nextLoop(){
		...
		if(document.all&&!window.opera) {for(i=0;i<ct;i++){al=((os+i)%ct); ele[al].style.filter="alpha(opacity="+parseInt(Math.min(1,Math.max(mo,1-((ct+1-i)/(ct+1))))*100)+")";}}
