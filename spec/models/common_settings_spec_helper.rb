...
# For model specs only.  Put controller specs in controller dir.
...
describe "a new object with categories", :shared => true do
  describe "with global category" do
     it "should belong to a global category, if selected" do
        @object.category.should be_global if @object.category
      end
    end
    
  describe "with user-created category" do
    it "should be non-global" do
      # Clear existing un-created 1st, if possible
      @object.category = nil
      @cat = create_category(:name=>"foo", :global => 0)
      @object.attributes = {:category_id => @cat.id}
      @object.save
...
describe "an object with common settings on update", :shared => true do
  ...
  describe "with time period" do
    it "should update associated time period attributes" do
      start_t = 1.day.ago
      end_t = Time.now
      @object.update_attributes(time_period_attributes(start_t, end_t))
      @object.beginning.day.should == start_t.day
      @object.end.day.should == end_t.day
    end
  end
  
  describe "with autorizations" do
    include ContentAuthorizationSpecHelper
    
    it "should update authorizations " do
      @circle = mock_model(Circle)
      guest1 = mock_model(Guest)
      guest2 = mock_model(Guest)
      guest1.stubs(:current_circle).returns(@circle)
      guest2.stubs(:current_circle).returns(@circle)
      Guest.stubs(:find).with(guest1.id).returns(guest1)
      Guest.stubs(:find).with(guest2.id).returns(guest2)
      @object.expects(:authorize_for_user).with(guest1, @circle)
      @object.expects(:authorize_for_user).with(guest2, @circle)
    
      @object.update_attributes(partial_privacy_attributes([guest1, guest2]))
    end
  end
end