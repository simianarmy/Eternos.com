...
describe Account do
  ...
  it 'should be invalid with an invalid user' do
    ...
    @account.errors.full_messages.should include("Password is too short")
  end
  
  it "should set the full domain when created" do
    @account = Account.create(:domain => 'foo', :user => new_user, :plan => create_subscription_plan)
  ...
  it "should require payment info when being created with a paid plan when the app configuration requires it" do
    @account = Account.new(:domain => 'foo', :user => new_user, :plan => @plan)
  ...
  it "should not require payment info when being created with free plan" do
    @account = Account.new(:domain => 'foo', :user => new_user, :plan => create_subscription_plan)
    @account.needs_payment_info?.should be_false
  end
  
  describe "when user account in initial state" do
    it "should be valid without valid payment and address info with a paid plan" do
      @account = Account.new(:domain => 'foo', :user => new_user, :plan => @plan)
      @account.valid?.should be_true
    end
  end
  
  describe "when user account in joined state" do
    it "should be invalid without valid payment and address info with a paid plan" do
      @account = Account.new(:domain => 'foo', :user => new_user, :plan => @plan)
      @account.state = 'joined'
  ...
  it "should create the subscription with an associated affiliate" do
    ...
    Subscription.all.last.affiliate.should == @affiliate
  end
  
  it "should have one admin user when created" do
    @account = Account.create(:domain => 'foo', :user => @user = new_user, :plan => create_subscription_plan)
  ...
  describe "validating domains" do
    ...
    it "should not allow the uniqueness check to interfere with updating an account" do
      @account = create_account(:full_domain => 'foo.bar')
  ...
  describe "when being created with payment info" do
    before(:each) do
      @account = Account.new(:domain => 'foo', :user => new_user, :plan => @plan, :creditcard => @card = CreditCard.new(valid_card), :address => @address = SubscriptionAddress.new(valid_address))
