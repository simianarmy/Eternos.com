  ...
  module Billing #:nodoc:
    ...
    class AuthorizeNetCimGateway < Gateway
      ...
      def commit(action, request)
        ...
        if !authorization && response_params['direct_response']
          authorization = response_params['direct_response'].split(',')[6]
        end
        
        response = AuthorizeNetCimResponse.new(success, message, response_params,
          :test => test_mode,
    ...
    class AuthorizeNetCimResponse < Response
      def token
        @authorization
      end
    end
