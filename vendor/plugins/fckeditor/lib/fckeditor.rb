...
module Fckeditor
  ...
  module Helper
    def fckeditor_textarea(object, field, options = {})
      # I am requiring that the 1st param is an object instance
      # Seems unnecessary to ask for a string instead...
      ...
      # Pass actual object, not object string since I changed _id methods
      # not to use eval but dom_id
      ...
      if options[:ajax]
        inputs = "<input type='hidden' id='#{id}_hidden' name='#{name}[#{field}]'>\n" <<
                 "<textarea id='#{id}' #{cols} #{rows} name='#{id}'>#{value}</textarea>\n"
      else 
        inputs = "<textarea id='#{id}' #{cols} #{rows} name='#{name}[#{field}]'>#{value}</textarea>\n"
      end
    ...
    def fckeditor_element_id(object, field)
      "#{dom_id(object)}_#{field}_editor"    
    end

    def fckeditor_div_id(object, field)
      "div-#{dom_id(object)}-#{field}-editor" 
    end
